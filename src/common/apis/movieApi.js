...
export const fetchHotMovies = async () => {	
	const response = await movieApi.get(`/discover/movie?api_key=${APIKey}&language=en-US$sort_by=popularity.desc&include_adult=false&include_video=false&with_original_language=en`).catch((err) => {
		console.log("Err :", err);
	});
...
export const fetchNewMovies = async () => {	
	const response = await movieApi.get(`/discover/movie?api_key=${APIKey}&language=en-US&sort_by=primary_release_date.desc&include_adult=false&include_video=false&release_date.lte=${date}&vote_count.gte=5&page=1&with_original_language=en`).catch((err) => {
		console.log("Err :", err);
	});
	return response.data.results
};
